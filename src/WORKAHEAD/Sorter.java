...
public class Sorter <T extends Comparable<T>> 
{
     ...
     */
    private void recursiveQuickSort(T[] array, int front, int back) {
        if (back <= front) return;
        else {
            int pivot = partition(array,front,back);
            recursiveQuickSort(array,front,pivot-1);
            recursiveQuickSort(array,pivot+1,back);
        }
    }
     ...
     * @param back the marker for the back partition 
     */
    private void recursiveQuickSort(LinkedList<T> list, int front, int back) {
        if (back <= front) return;
        else {
            int pivot = partition(list,front,back);
            recursiveQuickSort(list,front,pivot-1);
            recursiveQuickSort(list,pivot+1,back);
        }
    }
